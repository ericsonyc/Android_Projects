...
import com.android.androidpacman.app.Direction;
import com.android.androidpacman.app.PacmanGame;
import com.android.androidpacman.app.PacmanGame.GameplayMode;
...
public class Playfield extends BaseEntity {
        ...
        for (Path p : PATHS) {
            if (p.w > 0) {
                ...
                for (int x = p.x * 8; x <= (p.x + p.w - 1) * 8; x += 8) {
                    ...
                    pe.setPath(true);
                    if (pe.getDot() == PathElement.Dot.NONE) {
                        pe.setDot(PathElement.Dot.FOOD);
                        dotsRemaining++;
                    }
                    pe.setTunnel(
    ...
    private void createEnergizerElements() {
        ...
        for (Position c : ENERGIZER_POSITIONS) {
            ...
            playfield.get(Integer.valueOf(y))
                    .get(Integer.valueOf(x))
                    .setDot(PathElement.Dot.ENERGIZER);
        }
